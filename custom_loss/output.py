...
def decode_yolo_output(pred, stride, anchors):
    """
    Decode raw YOLO output to bounding box
    pred: [batch, 3, grid_h, grid_w, 85]
    stride: 8, 16 ỏ 32
    anchors: list of [w,h] pairs

    Returns:
        decode: [batch, 3, grid_h, grid_h, grid_w, 85]
    """
    batch_size, num_anchors, grid_h, grid_w, num_channels = pred.shape
    device = pred.device
